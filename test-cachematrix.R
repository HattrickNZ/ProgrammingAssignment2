...
test_that("cachematrix inverts a given matrix", {
  ...
  expect_that(cache$getmatrix(), equals(NULL))
  expect_that(cacheSolve(cache), equals(matAI))
  expect_that(cache$getmatrix(), equals(matAI))
  expect_message(cacheSolve(cache), "^getting cached matrix\\n$")
  
  cache$set(matB)
  expect_that(cache$get(), equals(matB))
  expect_that(cache$getmatrix(), equals(NULL))
  expect_that(cacheSolve(cache), equals(matBI))
  expect_that(cache$getmatrix(), equals(matBI))
  expect_message(cacheSolve(cache), "^getting cached matrix\\n$")
})